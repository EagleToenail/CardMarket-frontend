...
const UserImageButton = ({data, setIsOpenPointModal, setSpinFlag, check}) => {
  ...
  const submitDrawGacha = async (gacha, counts) => {
    ...
    const totalPoints =
      gacha.price *
      (counts === "all"
        ? (gacha.remain_prizes.filter((item) => item.order != 0).length + gacha.rubbish_total_number)
        : counts);
  ...
  const hasPrizes = data.remain_prizes.filter((item) => item.order !== 0).length + data.rubbish_total_number;
  return (
    <>
      {(hasPrizes === 0 || check) ? (
        <button
